...
// Utils
import { parseDateYYYYMMDD } from '../../utils/date';
...
export const NewTaskSheet: React.FC<NewTaskSheetProps> = ({
  ...
  const resetForm = useCallback(() => {
    console.log('[NewTaskSheet] Form reset triggered', { isVisible, listsCount: lists.length });
    setFormData({
      title: '',
      description: '',
      due_date: '',
  ...
  // Sheet açıldığında formu resetle (temiz başlangıç için)
  useEffect(() => {
    if (isVisible) {
      // Animasyon tamamlandıktan sonra resetle
      const timer = setTimeout(() => {
        resetForm();
      }, 100); // Animation başlangıcından sonra
          ...
          {labels.length > 0 && (
            <View style={styles.labelsContainer}>
              {labels.map((label) => {
                ...
                const labelTextStyle = isSelected ? { color: labelColor } : {};
