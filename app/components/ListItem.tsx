...
const ListItemComponent: React.FC<ListItemProps> = ({
  ...
  // const strikethrough = useSharedValue(isCompleted ? 1 : 0);
  ...
  const handleToggleComplete = () => {
    ...
    if (!isCompleted) {
      ...
      HapticFeedback.taskUncomplete();
    }

    // Animasyonlar devre dışı - stability için
    ...
    // if (!isCompleted) {
    //   // Completing task
    //   checkmarkScale.value = withSpring(1, { damping: 20, stiffness: 150 });
    //   strikethrough.value = withTiming(1, { duration: 300 });
    //   rowOpacity.value = withTiming(0.7, { duration: 200 });
    // } else {
    //   // Uncompleting task
    //   checkmarkScale.value = withSpring(0, { damping: 20, stiffness: 150 });
    //   strikethrough.value = withTiming(0, { duration: 200 });
  ...
  //   opacity: interpolate(strikethrough.value, [0, 1], [1, 0.6]),
  // }));
...
// Memoized ListItem with custom comparison
